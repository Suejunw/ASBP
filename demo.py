...
def main():
    ...
    parser.add_argument('--test_iter', default=250, type=int,
    ...
    parser.add_argument('--val_na_rate', default=0, type=int,
                        help='NA rate (NA = Q * na_rate)')
    ...
    if opt.pair:
        train_data_loader = get_loader_pair(opt.train, sentence_encoder,
                N=trainN, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size, encoder_name=encoder_name , mode='train')
        val_data_loader = get_loader_pair(opt.val, sentence_encoder,
                N=N, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size, encoder_name=encoder_name, mode='eval')
    ...
    if not opt.only_test:
        ...
        if ckpt is None:
            ...
            ckpt = 'none'
        if opt.only_test:
            acc = framework.eval(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair, test=False)
