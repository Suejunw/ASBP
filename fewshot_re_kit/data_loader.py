...
class FewRelDatasetPair(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # for word_query, word_support in zip(query, support*2):
        #         if self.encoder_name == 'bert':
        #             SEP = self.encoder.tokenizer.convert_tokens_to_ids(['[SEP]'])
        #             CLS = self.encoder.tokenizer.convert_tokens_to_ids(['[CLS]'])
        #             word_tensor = torch.zeros((self.max_length)).long()
...
class TestFewRelDatasetPair(data.Dataset):
    """
    FewRel Pair Dataset
    """
    ...
    def __getitem__(self, index):
        ...
        fusion_set = {'word': [], 'mask': [], 'seg': []}
        current = self.json_data[index]
        query_doc = current['meta_test']
        support_docs = current['meta_train']
        query_word = self.__getraw__(query_doc)
        for support_doc in support_docs:
            for support_d in support_doc:
                support_word = self.__getraw__(support_d)
                support.append(support_word)
        ...
        for word_support in support:
            if self.encoder_name == 'bert':
                ...
                CLS = self.encoder.tokenizer.convert_tokens_to_ids(['<s>'])
                word_tensor = torch.ones((self.max_length)).long()
            new_word = CLS + word_support + SEP + query_word + SEP
            for i in range(min(self.max_length, len(new_word))):
                word_tensor[i] = new_word[i]
            mask_tensor = torch.zeros((self.max_length)).long()
            mask_tensor[:min(self.max_length, len(new_word))] = 1
...
def get_loader_pair(name, encoder, N, K, Q, batch_size, 
        num_workers=8, collate_fn=collate_fn_pair, na_rate=0, root='./data', encoder_name='bert', mode='train'):
    if mode != 'test':
        ...
        dataset = TestFewRelDatasetPair(name, encoder, N, K, Q, na_rate, root, encoder_name)
        data_loader = data.DataLoader(dataset=dataset,
                                      batch_size=batch_size,
                                      shuffle=False,
                                      pin_memory=True,
