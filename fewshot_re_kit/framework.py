    ...
    def train(self,
        ...
        for it in range(start_iter, start_iter + train_iter):
            ...
            if (it + 1) % val_step == 0:
                ...
                #         count += 1
                #
                #     for iter2 in range(anchor_iter):
                #
                #         batch, label = next(self.val_data_loader)
                #         if torch.cuda.is_available():
                #             for k in batch:
                #                 batch[k] = batch[k].cuda()
                #             label = label.cuda()
                ...
                #
                #         sys.stdout.write(
                #             'step: {0:4} | loss: {1:2.6f}, accuracy: {2:3.2f}%, anchor: {3:} {4:}'.format(iter2 + 1,
                ...
                if acc > best_acc:
                    ...
                    best_acc = acc
                torch.save({'state_dict': model.state_dict()}, './checkpoint/last.pth.tar')
                iter_loss = 0.
                iter_loss_dis = 0.
                iter_right = 0.
                iter_right_dis = 0.
    ...
    def eval(self,
        ...
        if not test:
            ...
            with torch.no_grad():
                ...
                for it in range(len(eval_dataset)):
                    if pair:
                    ...
                    for p in pred:
                        if p != N:
                            ...
                            prediction.append(-1)
                    sys.stdout.write('[TEST] step: {0:4}'.format(it + 1) + '\r')
                    sys.stdout.flush()
                with open('result-{}-{}-{}.json'.format(str(N), str(K), str(na_rate)), 'w') as f:
